...
import 'package:project_files/widgets/fee_amount_collection_dialog.dart';
...
class _NewTripScreenState extends State<NewTripScreen> {
  ...
  endTripNow() async
  {
    ...
    // display fee amount in dialog box
    showDialog(
        context: context,
        builder: (BuildContext c) => FeeAmountCollectionDialog(
          totalFeeAmount: totalFeeAmount,
        ),
    );

    // add fee amount to deliveryman total earnings
    saveFeeAmountToDeliverymanEarnings(totalFeeAmount);
  }

  saveFeeAmountToDeliverymanEarnings(double totalFeeAmount)
  {
    FirebaseDatabase.instance.ref()
        .child("deliverymen")
        ...
        .once().then((snap)
    {
      //earnings sub child exists in database
      if(snap.snapshot.value != null)
        {
          double oldEarnings = double.parse(snap.snapshot.value.toString());
          double deliverymanTotalEarnings = totalFeeAmount + oldEarnings;

          FirebaseDatabase.instance.ref()
              .child("deliverymen")
              .child(currentFirebaseUser!.uid)
              .child("earnings")
              .set(deliverymanTotalEarnings.toString());
        }
      //earnings sub child does not exist in database
      else
        {
          FirebaseDatabase.instance.ref()
              .child("deliverymen")
  ...
  saveAssignedDeliverymanDetailsToUserCourierRequest()
  {
    ...
    databaseReference.child("car_details")
        .set("${onlineDeliverymanData.car_color} ${onlineDeliverymanData.car_model} - ${onlineDeliverymanData.car_number.toString()}");
